...
KB_DIR = pathlib.Path(r"C:\Protech\wpf-replay\kb")
EVAL = KB_DIR / "eng_eval.yaml"
...
ENV_FILE = REPO / ".env"
OUT = KB_DIR / "eng_eval_out_validation_round2.json"
CALL_TIMEOUT = 180     # 單題上限(秒);實測有題目穩定超過 90s 才回,不是卡死
INIT_TIMEOUT = 30
REQUEST_INTERVAL = 12  # Gemini quota throttle; one MCP call per 12 seconds
# ---------------------------------------------------------------------------
...
class MCP:
    ...
    def search(self, query):
        ...
        try:
            ...
            raise RuntimeError(f"MCP tool returned non-JSON content: {text[:200]}") from e
        ...
        for i, it in enumerate(items, 1):
            try:
                ...
                try:
                    ...
                    out, ok, note = {}, False, f"MCP error after restart/retry: {retry_error}"
